...
const alertVariants = cva(
  "relative w-full px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        default: "bg-accent text-card-foreground border border-foreground/10",
        destructive:
          "text-destructive bg-card border [&>svg]:text-current *:data-[slot=alert-description]:text-destructive/90",
        outline:
          "border bg-background shadow-xs dark:bg-input/30 dark:border-input",
        secondary:
          "bg-secondary border text-secondary-foreground shadow-xs",
        ghost:
          "bg-transparent",
      },
    },
